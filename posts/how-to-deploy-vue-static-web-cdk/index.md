---
title: "Deploy a Vue static website with the AWS CDK"
description: "A walkthrough on how to deploy a static website written in Vue with the AWS CDK"
tags:
    - aws
    - tutorials
    ...
    - vue
    - cdk
authorGithubAlias: deekob
authorName: Derek Bingham
date: 2023-10-01 
showInHomeFeed: true
---
Often as web developers we want a way to quickly deploy our existing static web applications, built with Vue, in the simplest and quickest way possible - without having to manage any complexities with underlying infrastructure, or gain knowledge of lots of AWS services.  However when we look for resources to show us how to do this, we are often led to articles that require a deep knowledge and understanding of AWS and its services and the learning curve is steep.  When we then add the requirement of an automated deployment mechanism for our website, this often leads to more complexity and knowledge of the inner workings of AWS.  We simply dont have time for this...

The good news is that there is a very easy way to deploy static websites on AWS, even with minimal knowledge of the inner workings and services of AWS itself.  We can achieve this by leveraging the AWS Cloud Development kit deploying our site to Amazon S3 and Amazon Cloudfront.

**[Amazon simple storage service (S3)](https://https://aws.amazon.com/s3/)** provides object storage that is highly durable, scalable and available. You can use it for web app storage, backup, data archival and even web site hosting.  

**[Amazon Cloudfront](https://aws.amazon.com/cloudfront/)** is a CDN that speeds up distribution of your static and dynamic web content, such as .html, .css, .js, and image files, to your users. CloudFront delivers your content through a worldwide network of data centers called edge locations. When a user requests content that you're serving with CloudFront, the request is routed to the edge location that provides the lowest latency (time delay), so that content is delivered with the best possible performance.
...
- Create a new website with Vue
- **optional** import an existing VUE website
- Use AWS CDK to:
  - package the web application source code
  - create the deployment infrastructure (using Amazon S3 and Amazon Cloudfront), and
...
| 💻 Code Sample         | Code sample used in tutorial on [GitHub](https://github.com/build-on-aws/aws-deploy-static-vue-website-cdk)                            |
...
* **An AWS account and CLI installed**: If you don't already have an account, follow the [Setting Up Your AWS Environment](https://aws.amazon.com/getting-started/guides/setup-environment/) guide for a quick overview and CLI installation steps.
...
The first thing we need to do is create a static web application in Vue.js that we will deploy to the cloud. 

The web application will be a simple web app server that will serve static HTML files and also have a REST API endpoint. 

The focus of this tutorial is not to teach you how to build Vue web applications, so feel free to use the example application or build your own one. 

While this tutorial focuses on using Vue.js you could use the same method for any a static site with an index.html entry point

You can implement all the folling steps on your local computer or in an [AWS Cloud9 environment](https://docs.aws.amazon.com/cloud9/latest/user-guide/tutorial.html?sc_channel=el&sc_campaign=devopswave&sc_content=cicdcdkebaws&sc_geo=mult&sc_country=mult&sc_outcome=acq).

### Create the client app

The first step is to create a new directory to house our application.

```bash
mkdir my_static_vue_app
cd my_static_vue_app
```

Next we will create a folder to store the code for the CDK and a folder that contains the static website assets.
...
Next, lets either copy in our existing Vue.js static web site - or if you dont have one, create an 'index.html' inside the 'web' directory 

Inside this file, add the following HTML 
...
Open the index.hml in your favourite browser to make sure it renders ok.


## Create Infrastructure Using AWS CDK

Now that we have our sample application, let's create a CDK application that will create all the necessary infrastructure to deploy the Vue.js web app

### Optional - Create GitHub Repository and personal access token
...
```bash
# Assuming at this point, you are inside the my_static_vue_app/web folder 
cd ..
mkdir cdk
cd cdk
```

Install the latest version of the CDK.
...
Now we will start to fill out the CDK code in the 'app.py' which is the entry point to the CDK code. Delete everything from the app.py file and lets go through and create it again step by step.

First lets import the base level CDK Contructs and give the CloudFormation Stack the CDK code will create a name. 
...
class CdkStack(Stack):
    ...
    #put your code here
stack_prefix = 'static-website-cloudfront-s3'
app = core.App()
stack = CdkStack(app, stack_prefix, stack_prefix=stack_prefix)
app.synth()
```

Here, ee have defined a CDK `CdkStack` class, this we will build out with the infrastructure we want to provision to host our website.

In this file `app.py`, we will write the code for all the resources stack we are going to create in this section. You can also copy-paste contents of this file from [here](https://raw.githubusercontent.com/build-on-aws/ TODO).
...
- **S3 Bucket**: 
- **S3 Origin Access Policy**: 
- **Cloudfront Distribution**:


### Setup the S3 Bucket 

For deploying the  app, we need to package it and upload it to [Amazon S3](https://aws.amazon.com/s3/?sc_channel=el&sc_campaign=devopswave&sc_content=cicdcdkebaws&sc_geo=mult&sc_country=mult&sc_outcome=acq)

To do that, we will be using a CDK construct called S3 Bucket. The S3 Bucket module will 

In the `app.py` file, add the dependency to the top of the file.
...
```python
    # S3 bucket to store the site assets
    s3_bucket = _s3.Bucket(
            self,
            id="{}-bucket".format(stack_prefix),
            bucket_name="demo-{}".format(stack_prefix),
...
This code uses the S3 Bucket module and does the following:

* gives the bucket a unique name and id ( Note: S3 bucket names must be globally unique)
* defines the removal policy of the bucket to be RemovalPolicy.DESTROY - this tells that CDK that if we ever remove this bucket from the stack to destroy it
* if the stack is destroyed also delete the bucket contents
* block public read access to the buccket and its contents

Once we have defined the S3 bucket, the next step is to copy the static web site assets into the bucket that was created.  To do this we do the following

In the `app.py` file, add the dependency to the top of the file.

```python
from aws_cdk import aws_s3_deployment as _s3deploy
```
Then we use the aws_s3_deployment CDK construct to deploy the correct assets into the bucket.  In this case we are using the entire contents of the 'web' folder created earlier.

```python
    # copy site assets into the bucket
    _s3deploy.BucketDeployment(self, id="{}-deployment".format(stack_prefix),
                                   sources=[_s3deploy.Source.asset(
                                       "../web")],
                                   destination_bucket=s3_bucket                                   
                                   )
```

## Setting up CloudFront for the Website

Next, we will use the AWS CDK to create the CDN for the site, using AWS Cloudfront. The first thing we need to do here is create a new origin access identity (OAI). An origin access identity is a special CloudFront user that you can associate with Amazon S3 origins, so that you can secure all or just some of your Amazon S3 content. For our website we want to restrict access to GET requests and we also need to setup Amazon Identity and Access Manager (IAM) to limit access to only a system user.  First lets import the CDK constructs we need.

```python
  from aws_cdk import aws_cloudfront as _cf
  from aws_cdk import aws_iam as _iam
```
Next then lets implememt these constructs in code **_Note_** The L2 CDK construct for cloudfront at time of writing only supports Origin Access Identities, which will be deprecated in CloudFront.  It is recommended to use Origin Access Control instead see **[GitHub Issue](https://github.com/aws/aws-cdk/issues/21771)** for more details

```python
  # setup Cloudfront Origin access
  cf_oai = _cf.OriginAccessIdentity(
      self, id="{}-cfn-oai".format(stack_prefix))

  oai_policy_statement = _iam.PolicyStatement(effect=_iam.Effect.ALLOW)
  oai_policy_statement.add_actions("s3:GetObject")
  oai_policy_statement.add_resources("{}/*".format(s3_bucket.bucket_arn))
  oai_policy_statement.add_principals(_iam.CanonicalUserPrincipal(
      cf_oai.cloud_front_origin_access_identity_s3_canonical_user_id))
  s3_bucket.add_to_resource_policy(oai_policy_statement)
```

Now that we have setup our access control for our bucket to only allow GETObject requests from CloudFront, the next step is to define the CloudFront Distribution.  A Cloudfront distribution, simply tells Cloudfront where you want the content for each request delivered from and in this case its always the bucket we have setup previously.  So lets configure the distribution in CDK code next.

First the construct imports, for the distribution we need to tell CloudFront where the origin of the content is and so we use the cloudfront_origins construct.

```python
  from aws_cdk import aws_cloudfront_origins as _cforigins
```

Once we have imported this, we setup the distribution by telling it where the entry point for our website is.  In this case it will be index.html. We also give it the bucket name and the access indentity rules that define access control for the bucket.

```python
  # setup Cloudfront distribution
        cf_cdn = _cf.Distribution(self,
                                  id="{}-cdn".format(stack_prefix),                                  
                                  default_root_object="index.html",
                                  default_behavior=_cf.BehaviorOptions(
                                      origin=_cforigins.S3Origin(bucket=s3_bucket, origin_access_identity=cf_oai))
                                  )
```

The last step in setting up CloudFront with the CDK is to output the URL that the website will be available at.

```python
# write the URL to an CFN output
        core.CfnOutput(self,
                       "{}-output-cloudfront".format(stack_prefix),
                       value=cf_cdn.distribution_domain_name,
                       export_name="{}-staticSite".format(stack_prefix))
